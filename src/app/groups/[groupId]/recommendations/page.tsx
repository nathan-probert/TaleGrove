...
export default function GroupRecommendationsPage() {
...
useEffect(() => {
  const loadBasicRecs = async () => {
    ...
    setIsLoading(true);
    try {
      const recs = await getGroupRecommendations(userId, groupId);
  ...
  // Only call loadBasicRecs when both userId and groupId are available
  if (userId && groupId) {
    loadBasicRecs();
  }
}, [userId, groupId]);
  ...
  const handleReject = async () => {
    ...
    if (rec) {
      try {
        await saveGroupBookRecommendation(userId, groupId, { title: rec.title, author: rec.author }, RecommendationStatus.Rejected);
      } catch (err: any) {
  ...
  const handleAccept = async () => {
    ...
    if (rec) {
      try {
        await saveGroupBookRecommendation(userId, groupId, { title: rec.title, author: rec.author }, RecommendationStatus.Accepted);
      } catch (err: any) {
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
          ...
          <div className="flex justify-center mt-8 gap-4">
            ...
            <button
              onClick={handleAdd}
              className="bg-purple-100 hover:bg-purple-200 text-purple-700 px-4 py-2 rounded-xl font-medium"
            >
